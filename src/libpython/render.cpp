...
// static BSDF *bsdf_getElement(BSDF *bsdf, int idx) {
// 	return cast(bsdf->getElement(idx));
// }
...
void export_render() {
	...
	BP_CLASS(Sensor, AbstractEmitter, bp::no_init)
		...
		.def("setSampler", &Sensor::setSampler)
	...
	BSDF *(BSDF::*bsdf_getElement)(void) = &BSDF::getElement;
	...
	BP_CLASS(BSDF, ConfigurableObject, bp::no_init)
		...
		.def("pdf", &BSDF::pdf)
		.def("getElement", bsdf_getElement, BP_RETURN_VALUE)
