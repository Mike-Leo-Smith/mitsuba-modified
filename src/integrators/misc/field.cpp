	...
	Spectrum Li(const RayDifferential &ray, RadianceQueryRecord &rRec) const {
		...
		switch (m_field) {
			...
			case ERelativePosition: {
					...
					Vector extend = aabb.getExtents();

					result.fromLinearRGB(p.x * 20.0 / extend.x * 0.025 + 0.5, p.y * 20.0 / extend.y  * 0.025 + 0.5 , p.z * 20.0 / extend.z  * 0.025 + 0.5);
				}
				break;
			case EDistance:
			...
			case EShadingNormal:{
					const Sensor *sensor = rRec.scene->getSensor();
					const Transform &t = sensor->getWorldTransform()->eval(its.t).inverse();
					Vector n = t(its.shFrame.n);
					result.fromLinearRGB(n.x * 0.5 + 0.5, n.y * 0.5 + 0.5, n.z * 0.5 + 0.5);
				}
				break;
			case EUV:
